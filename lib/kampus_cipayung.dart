                    ...
                    StreamBuilder<DatabaseEvent>(
                      ...
                      builder: (context, snapshot) {
                        ...
                        final gedungList = data.entries.map((entry) {
                          ...
                          final kelas = value['kelas'] as Map<dynamic, dynamic>? ?? {};
                          final lantaiEntries = kelas.entries.map((lantaiEntry) {
                            final lantaiValue = lantaiEntry.value as Map<dynamic, dynamic>? ?? {};
                            final roomEntries = lantaiValue.entries.map((roomEntry) {
                              final roomValue = roomEntry.value as Map<dynamic, dynamic>? ?? {};
                              return {
                                'name': value['slug'] as String? ?? entry.key as String, // Ambil dari slug di level gedung
                                'image': value['image'] is String ? value['image'] as String : '',
                              };
                            }).toList();
                            return roomEntries;
                          }).expand((x) => x).toList();
                          return lantaiEntries;
                        }).expand((x) => x).toList();
